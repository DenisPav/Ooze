...
public class DatabaseFilterInThanIntegrationTests(NpgsqlFixture fixture)
    : IClassFixture<NpgsqlFixture>
{
    ...
    public async Task Should_Correctly_Filter_Data_By_In_Int_Filter(params int[] postIds)
    {
        ...
        var validIds = castedIds.Where(x => x <= NpgsqlContext.TotalCountOfFakes);
...
class DateData : IEnumerable<object[]>
{
    public IEnumerator<object[]> GetEnumerator()
    {
        var dateChunks = Enumerable.Range(1, 101)
            .Select(x => new DateTime(2022, 1, 1, default, default, default, DateTimeKind.Utc).AddDays(x))
            .Chunk(25);
