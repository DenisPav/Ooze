    ...
    where TFixture : DbFixture
{
    ...
    [InlineData(25, 90)]
    [InlineData(95, 120)]
    public async Task Should_Correctly_Filter_Data_By_Range_Int_Filter_Async(
        int from,
        int to)
    {
        var filter = new RangeFilter<long> { From = from, To = to };
    ...
    [InlineData("-10", "5")]
    [InlineData("postname", "endpostname")]
    public async Task Should_Correctly_Filter_Data_By_Range_String_Filter_Async(
        string from,
        string to)
    {
        var filter = new RangeFilter<string> { From = from, To = to };
    ...
    [Fact]
    public async Task Should_Correctly_Filter_Data_By_Range_DateTime_Filter_Async()
    {
        var from = new DateTime(2022, 1, 1, 0, 0, 0, DateTimeKind.Utc);
        var to = new DateTime(2022, 2, 20, 0, 0, 0, DateTimeKind.Utc);
        var filter = new RangeFilter<DateTime>
